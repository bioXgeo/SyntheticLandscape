aacf <- function(rast, plot = TRUE) {
  # calculate the areal autocorrelation function
  
  # get raster dimensions
  M <- ncol(rast)
  N <- nrow(rast)
  
  zmat <- matrix(getValues(rast), ncol = M, nrow = N)
...
scl <- function(aacf_img, threshold = c(0.20, 1 / exp(1)), plot = TRUE) {
